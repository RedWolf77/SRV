...
void print_progress(int current, int total)
{
    ...
    if (pos > 0 && pos <= static_cast<size_t>(bar_width)) 
    {
        bar[pos - 1] = '>';
    }
...
int main()
{
    ...
    auto core = long_computation(TOTAL_STEPS);

    while (!core.done())
    {
        core.resume();
        int progress = core.get_value();
